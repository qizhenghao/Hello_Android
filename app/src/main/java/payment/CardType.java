...
public enum CardType {
    ...
    private static HashMap<Pair<String, String>, io.card.payment.CardType> intervalLookup;

    static {
        // initialize
        intervalLookup = new HashMap<Pair<String, String>, io.card.payment.CardType>();
        intervalLookup.put(getNewPair("2221", "2720"), io.card.payment.CardType.MASTERCARD);    // MasterCard 2-series
        intervalLookup.put(getNewPair("300", "305"), io.card.payment.CardType.DINERSCLUB);      // Diners Club (Discover)
        intervalLookup.put(getNewPair("309", null), io.card.payment.CardType.DINERSCLUB);       // Diners Club (Discover)
        intervalLookup.put(getNewPair("34", null), io.card.payment.CardType.AMEX);              // AmEx
        intervalLookup.put(getNewPair("3528", "3589"), io.card.payment.CardType.JCB);           // JCB
        intervalLookup.put(getNewPair("36", null), io.card.payment.CardType.DINERSCLUB);        // Diners Club (Discover)
        intervalLookup.put(getNewPair("37", null), io.card.payment.CardType.AMEX);              // AmEx
        intervalLookup.put(getNewPair("38", "39"), io.card.payment.CardType.DINERSCLUB);        // Diners Club (Discover)
        intervalLookup.put(getNewPair("4", null), io.card.payment.CardType.VISA);               // Visa
        intervalLookup.put(getNewPair("50", null), io.card.payment.CardType.MAESTRO);           // Maestro
        intervalLookup.put(getNewPair("51", "55"), io.card.payment.CardType.MASTERCARD);        // MasterCard
        intervalLookup.put(getNewPair("56", "59"), io.card.payment.CardType.MAESTRO);           // Maestro
        intervalLookup.put(getNewPair("6011", null), io.card.payment.CardType.DISCOVER);        // Discover
        intervalLookup.put(getNewPair("61", null), io.card.payment.CardType.MAESTRO);           // Maestro
        intervalLookup.put(getNewPair("62", null), io.card.payment.CardType.DISCOVER);          // China UnionPay (Discover)
        intervalLookup.put(getNewPair("63", null), io.card.payment.CardType.MAESTRO);           // Maestro
        intervalLookup.put(getNewPair("644", "649"), io.card.payment.CardType.DISCOVER);        // Discover
        intervalLookup.put(getNewPair("65", null), io.card.payment.CardType.DISCOVER);          // Discover
        intervalLookup.put(getNewPair("66", "69"), io.card.payment.CardType.MAESTRO);           // Maestro
        intervalLookup.put(getNewPair("88", null), io.card.payment.CardType.DISCOVER);          // China UnionPay (Discover)

        for (Entry<Pair<String, String>, io.card.payment.CardType> entry : getIntervalLookup().entrySet()) {
    ...
    private static HashMap<Pair<String, String>, io.card.payment.CardType> getIntervalLookup() {
     ...
     */
    public static io.card.payment.CardType fromString(String typeStr) {
        if (typeStr == null) {
            return io.card.payment.CardType.UNKNOWN;
        }

        for (io.card.payment.CardType type : io.card.payment.CardType.values()) {
            if (type == io.card.payment.CardType.UNKNOWN || type == io.card.payment.CardType.INSUFFICIENT_DIGITS) {
                continue;
            }
     ...
     */
    public static io.card.payment.CardType fromCardNumber(String numStr) {
        if (TextUtils.isEmpty(numStr)) {
            return io.card.payment.CardType.UNKNOWN;
        }

        HashSet<io.card.payment.CardType> possibleCardTypes = new HashSet<io.card.payment.CardType>();
        for (Entry<Pair<String, String>, io.card.payment.CardType> entry : getIntervalLookup().entrySet()) {
        ...
        if (possibleCardTypes.size() > 1) {
            return io.card.payment.CardType.INSUFFICIENT_DIGITS;
